...
description: >
  ...
  accent, the three-step gray text ladder, Montserrat Variable (display/numerals/chrome) + Montserrat Variable (body),
...
colors:
  ...
  text-muted: "#B5F7D6"
  text-light: "#9A9A9A"
  accent-light: "rgba(50, 108, 222, 0.08)"
  accent-medium: "rgba(50, 108, 222, 0.15)"
  border: "rgba(50, 108, 222, 0.2)"
  card-bg: "rgba(50, 108, 222, 0.04)"
  positive: "#059669"
  negative: "#dc2626"
...
typography:
  # — reading ramp (Montserrat Variable body + Montserrat Variable chrome) —
  body:    { fontFamily: "Montserrat Variable", cqw: 0.85, weight: 400, lineHeight: 1.6, color: "text-muted" }
  h4-eyebrow:{ fontFamily: "Montserrat Variable", cqw: 0.8, weight: 700, tracking: "0.08em", upper: true, color: "primary" }
  tag:     { fontFamily: "Montserrat Variable", px: 12, weight: 500, color: "primary" }
  counter: { fontFamily: "Montserrat Variable", px: 13, weight: 500, tracking: "0.05em", color: "text-muted" }
  # — display / numerical ramp (Montserrat Variable, near-black headings / cobalt numerals) —
  h3:      { fontFamily: "Montserrat Variable", cqw: 1.25, weight: 500, lineHeight: 1.3, tracking: "-0.02em", color: "text" }
  stat-num:{ fontFamily: "Montserrat Variable", cqw: 1.9, weight: 700, lineHeight: 1.0, color: "primary" }
  blockquote:{ fontFamily: "Montserrat Variable", cqw: 2.4, weight: 500, lineHeight: 1.35, color: "text" }
  h2:      { fontFamily: "Montserrat Variable", cqw: 2.6, weight: 700, lineHeight: 1.1, tracking: "-0.02em", color: "text" }
  metric-value:{ fontFamily: "Montserrat Variable", cqw: 3.0, weight: 700, lineHeight: 1.0, color: "primary" }
  h1:      { fontFamily: "Montserrat Variable", cqw: 4.2, weight: 700, lineHeight: 1.08, tracking: "-0.02em", color: "text" }
  quote-mark:{ fontFamily: "Montserrat Variable", cqw: 8.0, weight: 700, lineHeight: 0.5, color: "primary", opacity: 0.15 }
...
components:
  ...
  cta-button:
    ...
    textColor: "{colors.bg}"
    rounded: "{radii.pill}"
    typography: "Montserrat Variable 700"
...
- **Fonts** — already set to **Montserrat Variable** (display) / **Spectral** (body); ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{300, 400, 500, 700}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
The voice is two faces in fixed roles: **Montserrat Variable** (display, every numeral, all chrome —
eyebrows uppercase 0.08em) and **Montserrat Variable** (body, muted gray, line 1.6). Headlines are near-black;
...
- **Montserrat Variable** (display/numerals/chrome) + **Montserrat Variable** (body) — near-black headlines, cobalt numerals.
...
Two ramps. The **reading ramp** (Montserrat Variable body 0.85cqw muted; Montserrat Variable eyebrow uppercase 0.08em
cobalt) carries copy + chrome; the **display/numerical ramp** (Montserrat Variable `h3` 1.25cqw → `h1`
...
- **Headlines near-black, −0.02em**; **eyebrows cobalt, uppercase, 0.08em**; **numerals cobalt 600–700**; **body Montserrat Variable 400 muted, line 1.6**. No italic, no uppercase body, no cobalt headline.
...
**Composes** accent-line, meta, h1, body sub. **Focal** a 2-line Montserrat Variable `h1` near-black, left,
...
**Focal** a row of tinted cards — cobalt `metric-value` + Montserrat Variable label + muted desc + optional
...
blockquote, cite. **Focal** a Montserrat Variable `blockquote` near-black under a 15%-opacity cobalt
...
Montserrat Variable body column beside a cobalt-8% highlight block (4px cobalt left rule) carrying an inline pull
...
a Montserrat Variable `h1` near-black, centered, with the one solid cobalt `cta-button` pill below. **Accent**
...
- Use **tinted cards** (4% fill, 20% border, 10–14px radius, no shadow); body Montserrat Variable 400 muted, line 1.6.
...
- **Type** — Montserrat Variable headings −0.02em near-black, cobalt eyebrows 0.08em + numerals; Montserrat Variable body muted line 1.6; ≥1.4cqw floor.
...
- **Montserrat Variable + Montserrat Variable via Google Fonts.** CJK pairing (Noto Sans SC 700 display / Noto Serif SC 400 body) carries over; the eyebrow's uppercase+tracking signal weakens in CJK — pair it with the accent-line.
...
```html
<style>
@font-face{font-family:"Spectral";font-weight:300;font-style:normal;font-display:block;src:url("assets/fonts/Spectral-300.woff") format("woff");}
@font-face{font-family:"Spectral";font-weight:300;font-style:normal;font-display:block;src:url("assets/fonts/Spectral-300.woff2") format("woff2");}
@font-face{font-family:"Spectral";font-weight:300;font-style:italic;font-display:block;src:url("assets/fonts/Spectral-300-Italic.woff2") format("woff2");}
@font-face{font-family:"Spectral";font-weight:300;font-style:italic;font-display:block;src:url("assets/fonts/Spectral-300-Italic.woff") format("woff");}
</style>
```
